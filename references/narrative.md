...
`11px · uppercase · letter-spacing .14em · muted`
...
- **Vertical rhythm**: kicker → title → sub → key numbers → sections →
  ...
  (44–64px between major blocks, hairline separators above stats and
  chart blocks).
...
- **One nesting level per idea**: a section contains title → chart →
  takeaway, and stops there. If a section needs deeper nesting, split
...
- **takeaway is the one-liner reading** (serif, 17px, 2px accent left
  border): what this chart changes about the reader's picture.

Chart count follows the content: a long, evidence-rich input earns more
charts; two charts making the same point collapse into one.

### 6. Source line

`10px · uppercase · ls .1em · faint`

Every chart's provenance, and the page's aggregate source. Each source is
name + date + (URL if public). No source line = unfinished page (SKILL.md
rule 1).
